...
//
// What governs a call to a homeowner anywhere in the network:
//
//   The FTC's Telemarketing Sales Rule and the TCPA, which are federal and
//   apply in all five states. 8am–9pm in the CALLED PARTY's time zone, the
//   national Do Not Call registry, prompt identification of the caller and the
//   purpose, and an immediate, permanent stop on request.
//
//   State no-call registries and calling-hour rules, which are narrower than
//   federal in several states and are therefore what we actually enforce. The
//   conservative window below is tighter than the federal one everywhere, on
//   purpose: a single window that is legal in all five beats five windows and
//   a bug.
//
//   State telephone solicitation registration. Several states require a seller
//   to register and some require a bond. This is an open counsel question per
//   state — see docs/regions.md — and it gates cold calling, not door
//   knocking.
//
//   Recording consent. The callers sit in California, which requires all-party
//   consent, so all-party is the rule regardless of where the phone rings.
//
// Time zone is the field that actually matters and it is stored per lead:
// five states span Mountain, Central and Eastern. A 9am call to Indiana placed
// from a Denver desk at 9am is a 7am call, and that is a violation rather than
// a rounding error.
...
/**
 * The window we actually dial in, in the homeowner's own local time.
 *
 * Deliberately tighter than the federal 8am–9pm. Several states in the network
 * narrow the morning or the Sunday, and running one conservative window that
 * is lawful in all five is worth more than squeezing an extra hour out of the
 * loosest state and getting the lookup wrong once.
 */
...
export function windowFor(localTime: Date): CallWindow {
  ...
  if (day === 0) return CALLING_HOURS.sunday;
  if (day === 6) return CALLING_HOURS.saturday;
  return CALLING_HOURS.weekday;
}
...
export type DncList = "NATIONAL" | "STATE" | "INTERNAL" | "LITIGATOR";
...
export function screenLead(lead: ScreenableLead, now: Date, localTime: Date): Block[] {
      ...
      reason:
        list === "INTERNAL"
          ...
          : list === "LITIGATOR"
            ...
            : `On the ${list === "STATE" ? "state" : "national"} Do Not Call registry.`,
    });
  }
  ...
  if (!isWithinCallingHours(localTime)) {
    ...
    blocks.push({
      code: "OUTSIDE_HOURS",
      reason: `It is ${formatHour(localTime)} where they are. The window is ${formatClock(opens)}–${formatClock(closes)} today.`,
    });
  }
 ...
 *
 * Most network states are one-party. California (Penal Code 632) is all-party, and applies to
