// How a construction job actually gets paid for.
//
...
//
// Two rules shape the schedule rather than merely decorating it:
//
//   - Construction trust fund rules, which several states have in some form.
//      What a homeowner pays is earmarked for that job's labour and material.
//      Using a deposit as working capital for a different job is misapplication
//      of trust funds, criminal where the statute exists, and the single
//      easiest way for a growing contractor to end up in genuine trouble
//      without ever intending to. Colorado has an explicit version for roofing
//      payments taken before material is delivered.
//   - The federal three-day cooling-off right. A sale agreed at the kitchen
//      table can be cancelled for three business days in every state. Money
//      taken before that window closes is money that may have to go back, so
//      it does not get spent and the job does not get started.
//
...
import type { JobKind } from "@/lib/regions/contracts";
...
export const TRUST_RULE =
  ...
  "paid for. It is not working capital and it is not payroll for another job. Several states " +
  "in the network treat it as a trust fund and make spending it elsewhere a criminal offence " +
  "rather than a cash-flow decision.";
