...
// thing a state workforce agency reads if somebody files for unemployment
...
// clause in particular want an employment attorney in each state — see
// docs/regions.md.

import { COMPANY } from "@/lib/regions/brand";
import { DEFAULT_TERMS, type CommissionTerms } from "@/lib/regions/commission";
import { FEDERAL_COOLING_OFF_DAYS } from "@/lib/regions/compliance";
...
export function repAgreement(facts: RepAgreementFacts): AgreementDocument {
  ...
  if (terms.baseRate === 0) {
    warnings.push(
      "The base commission rate is 0%, so this agreement offers a rep nothing on the base " +
        ...
        "src/lib/regions/commission.ts, or per rep on the Rep row, before anyone signs.",
    );
  }
  if (!facts.w9OnFile) {
    warnings.push(
  ...
  const sections: ContractSection[] = [
    ...
    },
    {
      ...
      body:
        "These are not preferences. Each one is a statute somewhere in the network and breaking it exposes the " +
        ...
        `inside their ${FEDERAL_COOLING_OFF_DAYS} business days unless they ask in ` +
    ...
    },
    {
      ...
      body:
        `This agreement is governed by the law of the state the Representative works in. ` +
        `Notice to the Company goes to ` +
