...
export type MajorTradeProject = (typeof MAJOR_TRADE_PROJECTS)[number];
export type MajorTradeProjectValue = MajorTradeProject["value"];

export function isMajorTradeProject(value: string): value is MajorTradeProjectValue {
  return MAJOR_TRADE_PROJECTS.some((project) => project.value === value);
}

export function getMajorTradeLabel(value: string | null | undefined): string {
  if (!value) return "";
  return MAJOR_TRADE_PROJECTS.find((p) => p.value === value)?.label ?? value;
}
