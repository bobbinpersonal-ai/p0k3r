// The paper behind a Texas job: what the homeowner signs, and what the people
// doing the work sign.
//
...
//      crewAgreement.ts.
//
// Texas licenses no general contractor, remodeler or roofer, so nothing here
// turns on a licence number. What it turns on instead is a short list of
// statutes that bite hard when ignored, and the contract is where most of them
...
//   - Property Code Ch. 53 — a mechanic's lien on a Texas homestead needs a
//     written contract signed by BOTH spouses, executed BEFORE any work
//     starts, and filed with the county clerk. Get the order wrong and the
//     lien rights are simply gone.
...
//     misapplication of trust funds, which is a criminal offence in Texas.
//
...
// paragraph marked COUNSEL in docs/texas-contracts.md needs a Texas
// construction attorney's eyes before the first signature, not after.

import {
  ADJUSTER_LINE,
  CANCELLATION_BUSINESS_DAYS,
  ...
  formatLegalDate,
} from "@/lib/texas/compliance";
import { COMPANY, PROOF } from "@/lib/texas/brand";
import { getTrade, type Trade } from "@/lib/texas/trades";

export { CANCELLATION_BUSINESS_DAYS, REFUND_DAYS, cancellationDeadline, formatLegalDate };
 ...
 * construction trust funds is a criminal offence in Texas, and the threshold
 * for a felony is not high.
 */
export const TRUST_FUND_NOTICE =
  "Money you pay us for this project is held under Chapter 162 of the Texas Property Code " +
  "as a construction trust fund. It is used to pay for the labour and materials on your " +
  "project and for nothing else.";

/**
 * Property Code Ch. 53 — the homestead rules, which are unforgiving about
 * order of operations.
 *
 * For a lien to attach to a Texas homestead the contract must be in writing,
 * signed by both spouses if the owner is married, executed BEFORE any material
 * is delivered or any labour is performed, and filed with the county clerk.
...
export const HOMESTEAD_DISCLOSURE =
  "If this property is your homestead and you are married, Texas law requires that both " +
  "spouses sign this contract, that it be signed before any work begins or any material is " +
  "delivered, and that it be filed with the county clerk, in order for us to have lien " +
  "rights. We ask for both signatures for that reason. You are not giving up any homestead " +
  "protection by signing, and nothing in this contract waives it.";
 ...
 * Texas has a provision to this effect; I could not pin its exact citation
...
export function residentialContract(facts: ContractFacts): ContractDocument {
  ...
  const deadline = homeSolicitation ? cancellationDeadline(facts.signedAt) : null;
  ...
  if (facts.jobKind === "INSURANCE") {
    sections.push({
      ...
      body:
        ...
        "rebate, credit or absorb any part of it — Texas Business & Commerce Code § 27.02 " +
        "makes doing so a criminal offence, and anyone in this trade who offers it is " +
        "telling you something about how they do business.\n\n" +
        ADJUSTER_LINE +
        "\n\n" +
  ...
  sections.push({ heading: "If you think something is wrong", body: RCLA_NOTICE });
  ...
  if (homeSolicitation) {
    sections.push({
      ...
      body:
        `You agreed this contract at your home, so Texas Business & Commerce Code Chapter 601 ` +
        `gives you ${CANCELLATION_BUSINESS_DAYS} business days to cancel it for any reason at ` +
        `all — you do not have to give one. Cancel by midnight on ` +
  ...
  return {
    ...
    warnings,
    sections,
    proximityNotice: homeSolicitation ? CANCELLATION_PROXIMITY_NOTICE : null,
