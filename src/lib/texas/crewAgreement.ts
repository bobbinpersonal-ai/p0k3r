...
// twice or sues us. Texas gives that right by statute and the only clean answer
...
// specific text — have a Texas construction attorney confirm the four forms
// before the first one is signed. See docs/texas-contracts.md.

import { COMPANY, PROOF } from "@/lib/texas/brand";
import { SELLER, sellerAddress, type ContractSection } from "@/lib/texas/contracts";
import type { AgreementDocument } from "@/lib/texas/repAgreement";
...
export type CrewAgreementFacts = {
  ...
  /** Texas does not require it. Whether they carry it changes the exposure. */
...
export function crewAgreement(facts: CrewAgreementFacts): AgreementDocument {
  ...
  if (!facts.workersCompOnFile) {
    warnings.push(
      "No workers' compensation on file. Texas does not require it, so this is legal — but " +
        "it means a crew member hurt on one of our jobs has no compensation to claim and a " +
        "personal-injury suit is the only route left to them. Price that risk deliberately " +
        "or require coverage.",
    );
  }
  ...
  const sections: ContractSection[] = [
    ...
    },
    {
      ...
      body:
        ...
        (facts.workersCompOnFile
          ...
          : "The Contractor does NOT carry workers' compensation insurance. Texas does not " +
            "require it. The Contractor understands that neither they nor anyone they bring " +
            "onto a job is covered by any workers' compensation policy of the Company, that " +
            "the Company provides no medical or wage benefit for an injury on a job, and " +
            "that the Contractor is responsible for the safety of their own people. The " +
            "Contractor will tell everyone they bring onto a job the same thing."),
    },
    {
      heading: "Safety",
      body:
    ...
    },
    {
      ...
      body:
        `Texas law governs. Notice to the Company goes to ${SELLER.name}, ${sellerAddress()}. ` +
...
// --- Lien waivers ------------------------------------------------------------
//
// Texas prescribes four of these and a waiver that does not substantially
