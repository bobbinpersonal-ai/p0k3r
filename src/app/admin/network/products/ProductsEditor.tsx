...
import { TRADES, UNITS, type Tier } from "@/lib/regions/trades";
import { marginAtBase, type ProductConfigInput } from "@/lib/regions/productCatalogue";
...
export default function ProductsEditor({ initial }: { initial: Editable[] }) {
  ...
  async function refresh() {
    const res = await fetch("/api/admin/network/products");
  ...
  async function revert(product: Editable) {
    ...
    await fetch(
      `/api/admin/network/products?trade=${encodeURIComponent(product.trade)}&value=${encodeURIComponent(product.value)}`,
      { method: "DELETE" },
    );
    await refresh();
