...
import {
  ...
  sellerIsConfigured,
  type ContractFacts,
} from "@/lib/regions/contracts";
import { repAgreement } from "@/lib/regions/repAgreement";
import { crewAgreement, lienWaiver, type WaiverKind } from "@/lib/regions/crewAgreement";
import {
  OPENING_FEE_RATE,
  PRESENT_FEE_RATE,
  referralAgreement,
  TERM_LIMIT_DAYS,
} from "@/lib/regions/referralAgreement";
import { RAILS, feeFor, paymentSchedule, depositFor, TRUST_RULE, CANCEL_HOLD_RULE } from "@/lib/regions/payments";
...
function exampleJob(): ContractFacts {
  ...
  return {
    jobKind: "INSURANCE",
    // Colorado, because it is the strictest state in the network on insurance
    // roofing work — if the template holds up there it holds up everywhere.
    state: "CO",
      ...
      <div className="print:hidden">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {DOCS.map((d) => (
            <Link
              key={d.key}
              href={`/admin/network/paperwork?doc=${d.key}`}
          ...
          {doc && (
            <Link href="/admin/network/paperwork" className="rounded-full border border-white/15 px-3 py-1.5 text-xs text-neutral-300">
              Show all
            </Link>
          )}
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-4 font-mono text-xs uppercase tracking-widest">
          <Link href="/admin/network/products" className="text-neutral-300 underline">Products &amp; pricing</Link>
          <Link href="/admin/network/leads" className="text-neutral-300 underline">Leads</Link>
        <Link href="/admin/network/crews" className="text-neutral-300 underline">Crews</Link>
        ...
        <p className="mt-5 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs leading-relaxed text-neutral-300">
          Working templates built around the rules that actually bite across the network —
          the federal three-day cooling-off right, the deductible prohibitions in CO, KS, MO and
          IN, Kansas Attorney General roofing registration, and Indiana&apos;s Home Improvement
          Contracts Act. They are not legal advice and nobody here is your lawyer. The
          paragraphs flagged in <code className="font-mono">docs/regions.md</code> want a
          construction attorney in each state before the first signature.
        </p>
      </div>
