...
//
// Rendered by two routes and owned by neither. /channel-partners/portal reaches
// it with a session; /channel-partners/[token] reaches it with the link we
// texted, but only until that partner sets a password. Both hand it a partner
// id, and everything below is the same either way — a partner should not be
// able to tell which door they came through.
//
...
export default async function PortalView({
  partnerId,
  banner,
}: {
  partnerId: string;
  /**
   * The strip that says how you got here — "signed in as", or the invitation
   * to set a password. Passed in rather than decided here, because this
   * component deliberately cannot tell which door the visitor came through,
   * and rendered below the site header so it reads as part of the page
