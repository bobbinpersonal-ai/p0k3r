...
export default function EarningsEstimator({ feeCents }: { feeCents: number }) {
    ...
    <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 sm:p-8">
      ...
      <div className="mt-8 rounded-2xl border border-brand/30 bg-brand/10 p-6 text-center">
        ...
        <p className="mt-2 text-4xl font-extrabold tracking-tight text-ink sm:text-5xl">
          {money(lowJobs * fee)} – {money(highJobs * fee)}
        </p>
        <p className="mt-2 text-sm text-neutral-300">
          {lowJobs}–{highJobs} jobs at {money(fee)} each
        </p>
      </div>
      ...
      <p className="mt-5 text-xs leading-relaxed text-neutral-400">
        ...
        kind of houses they own move every one of these numbers. We&apos;d rather show you the
        range and the maths than a single number we picked because it sounded good.
      </p>
    </div>
  );
}
