...
- **`/book`** — customer booking form with an instant price range
- **`/book/confirmation`** — confirmation screen after a booking is submitted
- **`/admin`** — password-protected sign-in for dispatch
- **`/admin/dashboard`** — dispatch board: see incoming bookings, assign a driver,
  update status (Pending → Assigned → In Progress → Completed/Canceled), manage the
  driver roster
